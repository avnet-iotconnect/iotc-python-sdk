...
def get_kinesis_cer(cpid, uid, cacert, devicecert, devicekey):

    aws_endpoint = ""
    aws_role_alias = ""
    iot_cred_url = "https://" + aws_endpoint + "/role-aliases/" + aws_role_alias + "/credentials"
    print(iot_cred_url)
 
    try:
        # Mutual TLS: cert param is tuple (cert_file, key_file)
        # verify param is the path to the root CA
...
def stop_gstreamer():
    ...
    # stopprocess.wait()
    # print(f"Process returned with code: {stopprocess.returncode}")
