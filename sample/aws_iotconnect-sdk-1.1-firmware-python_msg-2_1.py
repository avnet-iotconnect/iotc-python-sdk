...
import threading
import random
from iotconnect import aws_IoTConnectSDK
...
SdkOptions={
	"certificate" : { 
        ...
        # iotcored1 Certs        
        "SSLKeyPath"  : "C:/Users/himanshu.parmar1/Downloads/iotcored1-certificates/pk_iotcored1.pem",    #aws=pk_devicename.pem   ||   #az=device.key
        "SSLCertPath" : "C:/Users/himanshu.parmar1/Downloads/iotcored1-certificates/cert_iotcored1.crt",    #aws=cert_devicename.crt ||   #az=device.pem
        "SSLCaPath"   : "C:/Users/himanshu.parmar1/Downloads/iotcored1-certificates/AmazonRootCA1.pem"     #aws=root-CA.pem         ||   #az=rootCA.pem  
...
def main():
    ...
    try:
        ...
        * Output  : Callback methods for device command and twin properties
        """
        with aws_IoTConnectSDK( UniqueId, SId, cpid, env, pf, SdkOptions, DeviceConectionCallback) as Sdk:
            try:
                """
                ...
                while True:
                    ...
                    * Non Gateway device input data format Example:
                    ...
                    dObj =  [ 
                                {
                                    "uniqueId":UniqueId,
                                    "time": datetime.now().astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.000Z"),
                                    "data": 
                                        {
                                            "rssi": random.randint(1, 20)
                                        }
                                }
                            ]
